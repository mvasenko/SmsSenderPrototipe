...
namespace BLL.Implementations.Services
{
    public class SendInvitesRequestValidationService : ISendInvitesRequestValidationService<SendInvitesDto>
    {
        public void ValidateRequest(SendInvitesDto dto)
        {
            ...
            if (new HashSet<string>(dto.PhoneNumbers).ToList().Count < dto.PhoneNumbers.Count)
            {
        ...
        {
            if (currentInvitationsNumber + dto.PhoneNumbers.Count > 128)
            {
